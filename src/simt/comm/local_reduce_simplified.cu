...
__global__ void reduce(int* in_array, int* out_array, size_t size) {
    ...
    shared_array[threadIdx.x] = A(in_array[thread_id]);
    ...
    for(size_t i=1; i<BLOCK_SIZE; i<<=1){
        if((i&threadIdx.x) == 0){
            ...
            shared_array[threadIdx.x] = B(shared_array[threadIdx.x],shared_array[other_index]);
        }
        __syncthreads();
    }

    if(threadIdx.x == 0){
        out_array[blockIdx.x] = shared_array[threadIdx.x];
    }
}
