...
__global__ void reduce(int* in_array, int* out_array, size_t size) {
    ...
    int local_value = in_array[thread_id];
    if((threadIdx.x&1) != 0){
...
int main(int argc, char *argv[]) {
    ...
    map<<<out_size,GROUP_SIZE>>>(in_array,size);

    for(size_t i=size; i>1; i=(i+GROUP_SIZE-1)/GROUP_SIZE){
        print_array(in_array,size);

        reduce<<<out_size,GROUP_SIZE>>>(in_array,out_array,size);
        auto_throw(cudaDeviceSynchronize());

        // Re-arrange and re-allocate buffers for next reduction
        if(out_size > 1){
            auto_throw(cudaFree(in_array));
            in_array = out_array;
            size = out_size;
            out_size   = (size+GROUP_SIZE-1) / GROUP_SIZE;
            auto_throw(cudaMallocManaged(&out_array,out_size*sizeof(int)));
        }
    }
